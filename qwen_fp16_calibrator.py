...
class AdaptiveScaledAttentionWrapper:
    """Attention wrapper with configurable per-layer scaling"""
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, 
        ...
        if not is_fp16 or (self.qkv_scale == 1.0 and self.out_scale == 1.0):
            return self.original_processor(attn, hidden_states, encoder_hidden_states, 
        ...
        if self.qkv_scale != 1.0:
            attn.to_q = lambda x: original_to_q(x / self.qkv_scale) * self.qkv_scale
            attn.to_k = lambda x: original_to_k(x / self.qkv_scale) * self.qkv_scale
            attn.to_v = lambda x: original_to_v(x / self.qkv_scale) * self.qkv_scale
        
        if self.out_scale != 1.0:
            attn.to_out[0] = lambda x: original_to_out_0(x / self.out_scale) * self.out_scale
        
        # Call original processor
        result = self.original_processor(attn, hidden_states, encoder_hidden_states, 
                                        attention_mask, **kwargs)
...
class QwenFP16Calibrator:
    ...
    def auto_calibrate(self, test_prompt="A coffee shop with neon lights", 
        ...
        for iteration in range(max_iterations):
            ...
            # Test generation
            success, image = self.test_generation(test_prompt, steps=20, width=1024, height=1024)
